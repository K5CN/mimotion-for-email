...
def get_time():
    try:
        ...
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36","Connection": "close"}
        s = requests.session()
        s.keep_alive = False
...
# 主函数
def step_modify(_user, _passwd, _step):
    user = str(_user)
    password = str(_passwd)
    step = str(_step)
    # step = str(26702)
    print("已设置为步数" + str(step))
    if user == '' or password == '':
        print("用户名或密码填写有误！")
        return
...
if __name__ == "__main__":
    ...
    password = "xxx"
    step="3000"
    step_modify(user_id,password,step)
